...
if [ ! -f ~/.codex/maestro-source ]; then
  echo "ERROR: Maestro source path not found."
  echo "This usually means Maestro was installed before this feature was added."
  echo ""
  echo "To fix, run install.sh manually from your codex-maestro repo:"
  echo "  cd /path/to/codex-maestro && ./install.sh"
  exit 1
fi

# Get source path
MAESTRO_SOURCE=$(cat ~/.codex/maestro-source)
...
1. Reads the saved Maestro source path from `~/.codex/maestro-source`
...
- If you move the codex-maestro repo, run `install.sh` again from the new location
