...
1. **Check for existing state**: Read `.codex/product-state.json` if it exists
...
Read the workflow definition from `~/.codex/product-steps.json` and templates from `~/.codex/product-templates.json`.

### If no state file exists (new service):

1. Create `.codex/product-state.json` with initial state
