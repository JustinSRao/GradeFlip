...
SPRINT_NUM=$(python3 ~/.codex/scripts/sprint_lifecycle.py next-sprint-number --dry-run | grep "Next sprint number" | awk '{print $4}')
...
if [ -n "$EPIC" ]; then
  SPRINT_NUM=$(python3 ~/.codex/scripts/sprint_lifecycle.py register-sprint "$TITLE" --epic $EPIC --estimated-hours $HOURS 2>&1 | grep "Registered sprint" | awk '{print $3}')
else
  SPRINT_NUM=$(python3 ~/.codex/scripts/sprint_lifecycle.py register-sprint "$TITLE" --estimated-hours $HOURS 2>&1 | grep "Registered sprint" | awk '{print $3}')
fi
