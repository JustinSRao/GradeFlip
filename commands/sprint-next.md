...
```bash
python3 ~/.codex/scripts/sprint_lifecycle.py advance-step $ARGUMENTS
```
...
1. **Read State**: Loads `.codex/sprint-{N}-state.json` to get current step
...
See `~/.codex/sprint-steps.json` for complete workflow definition with 4 phases:
