...
Implement the local storage contract for GradeFlip so decks, flashcards, notes, and images can be stored predictably on device, protected locally, and wired into the app scaffold for later synced use.
...
- [ ] Apply early local data-protection hooks suitable for Apple-platform file storage
- [ ] Support future migration/versioning of stored deck formats
- [ ] Scaffold the app-facing persistence integration points needed to exercise stored content later
...
- [ ] The implementation must be compatible with stronger encryption or protected-storage enhancements later
...
- App-shell wiring for local storage entry points
...
Write persistence services that validate models before save, write to a temporary file first, then replace the live file. Store image references by generated asset IDs rather than original filenames. Include file-protection-ready storage abstractions from the first implementation slice, and wire the resulting storage layer into the app scaffold rather than leaving it as package-only code.
...
- [ ] Define the first app-facing persistence integration surface
...
- [ ] Implement data-protection-ready local storage hooks
- [ ] Scaffold app-layer storage bootstrap and sample loading flows
...
- [ ] Validate that protected-storage assumptions do not block normal offline usage
...
- [ ] The app scaffold has a defined way to consume local persisted content
