...
State files are stored in .codex/sprint-{N}-state.json
"""
...
def get_state_file_path(project_root: Path, sprint_num: int) -> Path:
    ...
    Returns:
        Path to state file
    """
    return project_root / ".codex" / f"sprint-{sprint_num}-state.json"
...
def load_workflow_steps() -> Dict:
    """
    Load workflow steps definition from ~/.codex/sprint-steps.json
