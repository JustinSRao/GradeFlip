...
Maestro (Codex workflow) and Auto Codex (autonomous agent pipeline)
can use.

State Location: {project}/.codex/task-state.json

This module is duplicated in Auto Codex backend (apps/backend/spec/unified_state.py)
...
PROJECT_MARKERS = [".git", ".auto-codex", "package.json", "pyproject.toml", ".codex"]


def find_project_root(start_path: Path | None = None) -> Path | None:
    """Walk up directory tree to find project root.

    Looks for common project markers (.git, .auto-codex, package.json, etc.)
...
def get_state_path(project_root: Path | None = None) -> Path:
    ...
    {project}/.codex/task-state.json. Otherwise falls back to
    ~/.codex/task-state.json for non-project contexts.
...
def load_state(project_root: Path | None = None) -> dict[str, Any]:
    ...
    Loads the task state from {project}/.codex/task-state.json with
